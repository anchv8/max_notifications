...
| `REPORT_TIME` | ❌ | Время ежедневной отправки отчёта о бэкапах администраторам в формате `HH:MM`. Например: `09:00`. Если не задан — автоотчёт отключён |
...
#### `/report`
Немедленно сформировать и отправить ежедневный отчёт о бэкапах. Показывает успешные задачи и все проблемы (ошибки + пропуски) за последние 24 часа.

Автоматический отчёт настраивается через `REPORT_TIME` в `.env`.
